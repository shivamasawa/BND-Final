...
import mobile_util, webdriver_util
import datetime
...
def verifyElementIsDisplayedAndSpansWidth(self, web_element):
    ...
    assert (width <= browser_width) and (width >= (browser_width - 80))

class OptimizedTemplateTestCase(test_mobile_core.MobileTestCore):
    ...
    def testOptimizedTemplate(self):
        ...
        try:
            ...
            print("testWidget: pass")
        except Exception as err:
            print("testWidget: FAIL")
            print("Error URL: "+self.driver.current_url)
            print(err)
            errors.append(err)
        print("---------------------------------------------------")
